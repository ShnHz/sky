...
<template>
  <div :style="{'marginTop':marginTop}" class="empty-wrap">
    <div class="empty">
      <div :style="{'height':image == 'PRESENTED_IMAGE_SIMPLE' ? '40px' : ''}" class="empty-image">
        <p v-html="image == 'PRESENTED_IMAGE_SIMPLE' ? PRESENTED_IMAGE_SIMPLE : PRESENTED_IMAGE_DEFAULT" v-if="image == 'PRESENTED_IMAGE_SIMPLE' || !image"></p>
  ...
  <!-- 
    <empty image="PRESENTED_IMAGE_SIMPLE">
      <span slot="description">Customize</span>
    </empty>
  -->
</template>
<script>
